			...
			if ((child.type == ClassNode.CLASS_LOCAL || child.type == ClassNode.CLASS_ANONYMOUS) && child.enclosingMethod == null) {
				...
				if (setEnclosing.size() == 1) {
					...
					if (attr != null && attr.getMethodName() != null) {
						if (node.classStruct.qualifiedName.equals(attr.getClassName()) && node.classStruct.getMethod(attr.getMethodName(), attr.getMethodDescriptor()) != null) {
			...
			if (meth.root != null) { // neither abstract nor native
				...
				graph.iterateExprents(new DirectGraph.ExprentIterator() {
					public int processExprent(Exprent exprent) {
						...
						if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
							...
							if (asexpr.getLeft().type == Exprent.EXPRENT_FIELD) {
								...
								if (fexpr.getClassname().equals(child.classStruct.qualifiedName) &&  // process this class only
										mapFieldsToNewVars.containsKey(InterpreterUtil.makeUniqueKey(child.classStruct.qualifiedName, InterpreterUtil.makeUniqueKey(fexpr.getName(), fexpr.getDescriptor().descriptorString)))) {
									return 2;
								}
						...
						if (child.type == ClassNode.CLASS_ANONYMOUS && CodeConstants.INIT_NAME.equals(meth.methodStruct.getName()) && exprent.type == Exprent.EXPRENT_INVOCATION) {
