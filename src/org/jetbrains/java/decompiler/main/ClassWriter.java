...
public class ClassWriter {
	...
	private static void addTracer(StructClass cls, StructMethod method, BytecodeMappingTracer tracer) {
		StructLineNumberTableAttribute lineNumberTable = (StructLineNumberTableAttribute) method.getAttributes().getWithKey(StructGeneralAttribute.ATTRIBUTE_LINE_NUMBER_TABLE);
		tracer.setLineNumberTable(lineNumberTable);
		DecompilerContext.getBytecodeSourceMapper().addTracer(cls.qualifiedName, InterpreterUtil.makeUniqueKey(method.getName(), method.getDescriptor()), tracer);
	}
	...
	public void classToJava(ClassNode node, TextBuffer buffer, int indent, BytecodeMappingTracer tracer) {
		...
		try {
			...
			for (StructField field : cl.getFields()) {
				...
				boolean isEnum = field.hasModifier(CodeConstants.ACC_ENUM) && DecompilerContext.getOption(IFernflowerPreferences.DECOMPILE_ENUM);
				if (isEnum) {
					if (enumFields) {
						buffer.append(',').appendLineSeparator();
	...
	private void fieldToJava(ClassWrapper wrapper, StructClass clazz, StructField field, TextBuffer buffer, int indent, BytecodeMappingTracer tracer) {
		int start = buffer.length();
		boolean isInterface = clazz.hasModifier(CodeConstants.ACC_INTERFACE);
		boolean isDeprecated = field.getAttributes().containsKey("Deprecated");
		boolean isEnum = field.hasModifier(CodeConstants.ACC_ENUM) && DecompilerContext.getOption(IFernflowerPreferences.DECOMPILE_ENUM);
		...
		if (interceptor != null) {
			String oldName = interceptor.getOldName(clazz.qualifiedName + " " + field.getName() + " " + field.getDescriptor());
		...
		appendAnnotations(buffer, field, indent);

		buffer.appendIndent(indent);

		if (!isEnum) {
			appendModifiers(buffer, field.getAccessFlags(), FIELD_ALLOWED, isInterface, FIELD_EXCLUDED);
		}

		VarType fieldType = new VarType(field.getDescriptor(), false);

		GenericFieldDescriptor descriptor = null;
		if (DecompilerContext.getOption(IFernflowerPreferences.DECOMPILE_GENERIC_SIGNATURES)) {
			StructGenericSignatureAttribute attr = (StructGenericSignatureAttribute) field.getAttributes().getWithKey("Signature");
			if (attr != null) {
		...
		if (field.hasModifier(CodeConstants.ACC_STATIC)) {
			initializer = wrapper.getStaticFieldInitializers().getWithKey(InterpreterUtil.makeUniqueKey(field.getName(), field.getDescriptor()));
		} else {
			initializer = wrapper.getDynamicFieldInitializers().getWithKey(InterpreterUtil.makeUniqueKey(field.getName(), field.getDescriptor()));
		}
		if (initializer != null) {
			...
			StructConstantValueAttribute attr = (StructConstantValueAttribute) field.getAttributes().getWithKey(StructGeneralAttribute.ATTRIBUTE_CONSTANT_VALUE);
			if (attr != null) {
				PrimitiveConstant constant = clazz.getPool().getPrimitiveConstant(attr.getIndex());
				buffer.append(" = ");
