	...
	public void setVarDefinitions() {
		...
		for (Entry<Integer, Statement> en : mapVarDefStatements.entrySet()) {
			...
			if (implDefVars.contains(index)) {
				...
				//System.out.println("Ignore me we were kinda close but ye");
				continue;
			}
			...
			if (!defset) {
				...
				var.setDefinition(true);
				//System.out.println("ok " + var + ", " + var.getVarType() + ", " + var.toJava(0, new BytecodeMappingTracer()));

				lst.add(addindex, var);
			} else {
				//System.out.println("damn def set");
			}
		}
	}
