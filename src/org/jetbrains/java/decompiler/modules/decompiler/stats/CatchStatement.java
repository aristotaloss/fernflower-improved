...
public class CatchStatement extends Statement {
	...
	private CatchStatement(Statement head, Statement next, HashSet<Statement> setHandlers) {
		...
		for (StatEdge edge : head.getSuccessorEdges(StatEdge.TYPE_EXCEPTION)) {
			...
			if (setHandlers.contains(stat)) {
				...
				vars.add(new VarExprent(DecompilerContext.getCounterContainer().getCounterAndIncrement(CounterContainer.VAR_COUNTER), new VarType(CodeConstants.TYPE_OBJECT, 0, edge.getExceptions().get(0)),
	...
	public Statement getSimpleCopy() {
		...
		for (List<String> exc : this.exctstrings) {
			...
			cs.vars.add(new VarExprent(DecompilerContext.getCounterContainer().getCounterAndIncrement(CounterContainer.VAR_COUNTER), new VarType(CodeConstants.TYPE_OBJECT, 0, exc.get(0)), (VarProcessor) DecompilerContext.getProperty(DecompilerContext.CURRENT_VAR_PROCESSOR)));
		}
