	...
	public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
		...
		switch (looptype) {
			...
			case LOOP_FOR:
				...
				Exprent incrFnc = incExprent.get(0);
				if (incrFnc instanceof FunctionExprent && ((FunctionExprent) incrFnc).getFuncType() == FunctionExprent.FUNCTION_PPI) {
					((FunctionExprent) incrFnc).setFuncType(FunctionExprent.FUNCTION_IPP);
				}

				buf.append("; ").append(conditionExprent.get(0).toJava(indent, tracer)).append("; ").append(incrFnc.toJava(indent, tracer)).append(") {").appendLineSeparator();
				tracer.incrementCurrentSourceLine();
				buf.append(ExprProcessor.jmpWrapper(first, indent + 1, true, tracer));
				buf.appendIndent(indent).append("}").appendLineSeparator();
				tracer.incrementCurrentSourceLine();
